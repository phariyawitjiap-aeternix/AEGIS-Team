...
- **Agent**: Navi (opus) — ceremony facilitator and orchestrator
...
- [ ] Code review approved by @vigil (Gate 1)
- [ ] QA verdict PASS from @sentinel (Gate 2) — for tasks >= 3 pts
- [ ] Documentation updated if user-facing behavior changed
- [ ] Kanban status updated to DONE
- [ ] Relevant ISO 29110 docs updated by @scribe (Gate 3) — if compliance mode active
```
...
Last updated: <YYYY-MM-DD HH:MM> by Navi
...
The standup is auto-generated from agent activity. Navi reads the logs and produces the standup — no manual input required from the team.
...
**Sprint**: <N> | **Day**: <D> of <total_days> | **Generated by**: Navi (auto)
...
|------|------|----|-------|------|
| T-XXX | IN_PROGRESS | IN_REVIEW | @bolt | <timestamp> |

---

## Notes

<Any notable context from Mother Brain or Navi>
```
...
The sprint review summarizes completed work and links to all outputs produced. It is written by Navi and acts as the formal demo record.
...
|----|------|--------|----------|---------------------|
| T-001 | <description> | <pts> | @bolt | [link or path to output] |
...
|---|--------|-------|--------------|
| 1 | <specific, actionable change> | @<agent or Navi> | Sprint <N+1> |
| 2 | <specific, actionable change> | @<agent or Navi> | Sprint <N+1> |
...
|-----------|-----------|-------------|
| BACKLOG → TODO | Sprint planning assigns it | Navi at sprint start |
| TODO → IN_PROGRESS | Agent picks it up | Navi (on agent's behalf) |
| IN_PROGRESS → IN_REVIEW | Agent completes implementation | Bolt / implementing agent |
| IN_REVIEW → QA | Vigil approves code review (Gate 1 PASS) | Vigil |
| IN_REVIEW → IN_PROGRESS | Vigil rejects code review (Gate 1 FAIL) | Vigil |
| QA → DONE | Sentinel approves QA verdict (Gate 2 PASS) | Sentinel |
| QA → IN_PROGRESS | Sentinel fails QA verdict (Gate 2 FAIL, with findings) | Sentinel |
| Any → TODO | Task becomes blocked | Navi |
...
**Single-writer rule**: Only Navi writes to `kanban.md`. All other agents send a StatusUpdate message; Navi performs the actual column move. This prevents concurrent write conflicts.
