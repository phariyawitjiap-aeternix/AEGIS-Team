...
#   --lint   on the live tree (38 skills, all 9 keys present)
...
# ─── T1: live tree passes ──────────────────────────────────────────────────
echo
echo "T1: live tree --lint passes (38 skills, all 9 keys)"
...
if echo "$OUT" | grep -q "all 38 skills satisfy schema"; then
    pass "live tree reports 38 skills satisfy"
else
    fail "live tree reports 38 skills satisfy" "output:\n$OUT"
fi
