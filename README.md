...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/skills-40-orange?style=for-the-badge" alt="40 Skills"/>
...
>
> :dna: Nick Fury · 11 Marvel Agents · 16 canonical commands · 39 Skills · 14 Hooks · 60+ Helper Tools · NDJSON knowledge graph · Linear hub mirror · ISO 29110 · Diagram-first reflex · CC 2.1.141 native · [`PROJECT_INDEX.md`](PROJECT_INDEX.md) (auto-generated by v12-06)

---

## :bar_chart: Benchmark — SWE-bench Verified (2026-05-31)

**429 / 500 resolved (85.8%)** on the full SWE-bench Verified set. Official
harness, scored across 10 parallel x86 jobs ([`swe-bench-eval-batched.yml`](.github/workflows/swe-bench-eval-batched.yml)),
verified by summing all 10 chunk reports. Full breakdown + reproducibility:
[`_aegis-output/benchmarks/swe-bench/RESULTS.md`](_aegis-output/benchmarks/swe-bench/RESULTS.md).

> **Read this honestly:** the number measures **Claude Opus 4.8's raw coding
> capability through a thin harness** (`run_agent.py` = "make the minimal fix" +
> `claude -p`). It does **not** exercise AEGIS's personas, quality gate, MBP, or
> brain — so it is a model-capability reference, not a measure of AEGIS's
> orchestration value-add. AEGIS's actual differentiation is trust / enforcement
> / audit (reliability and verifiability), which this benchmark does not score.
> 15 instances did not complete the eval and are counted as unresolved
...
- Skill frontmatter schema (5 graph keys: reads/writes/wires/tests/supersedes), backfilled across 40 skills
...
| 5 | :package: **Install** 11 Marvel agents, 16 canonical commands, references + teams, 40 skills, 14 hooks, 60+ helper tools |
...
├── CLAUDE_skills.md             # Skill catalog (40 skills, 3 profiles)
...
### v15-28 (current) — SWE-bench benchmark + non-stop execution + native-tune (2026-05-28..31)

| Area | Pt | Headline |
|---|---:|---|
| Benchmark | 8 | SWE-bench Verified **429/500 (85.8%)** — agent harness (`run_agent.py`) + batched x86 eval; resilient to network/auto-update/rate-limit via skip-and-resume hardening. See the honesty caveat in the Benchmark section above. |
| Non-stop exec | 5 | `aegis-autopilot.sh` (headless Ralph-loop) + `aegis-daemon.sh` (TUI auto-restart) — work continues across sessions without a human `/aegis-start`. |
| Quality | 4 | `aegis-quality-gate.sh` (review + test + spec, `--parallel`) wired into Nick Fury BLOCK 6 — verdict is the source of truth before DONE. |
| Continuity | 3 | `aegis-checkpoint.sh` (structured resume) + `aegis-credential-scan.sh` (ask-once intake gate). |
| MBP | 2 | Round 4 (unnecessary-confirmation) + Round 5 (recap/Next: doable-actions) patterns; test now sources the real hook (drift killed). |
| Native-tune | 3 | Removed unwired `aegis-router`; refreshed 4 opus persona pins 4.6→4.8; installer upgrade-sweep manifest guard (was deleting project skills). Plan: [`docs/AEGIS_NATIVE_TUNE_PLAN.md`](docs/AEGIS_NATIVE_TUNE_PLAN.md). |

### v15.1.0 — Honesty contracts + lean pass + upgrade-sweep (8 sprints across 2 days, 2026-05-22..23)
...
| v12-03 | 5 | Skill frontmatter schema (5 graph keys) + backfill across 40 skills |
