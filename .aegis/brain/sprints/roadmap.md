...
| sprint-v15-15-pretooluse-modern-schema (PreToolUse "Bash hook error" / "Write hook error" labels) | 5 | 5 | 0 | **CLOSED (100%)** — 3 PreToolUse hooks (approval-gate, guard-bash, guard-write) migrated from dual-path (modern JSON + stderr + exit 2) to modern-only (JSON + exit 0). Eliminates CC 2.1.143's red "hook error" label on legitimate blocks. `AEGIS_APPROVAL_GATE_LEGACY=1` and `AEGIS_GUARD_LEGACY=1` env vars opt-in for older CC versions. Test helpers refactored to detect blocks via JSON (implementation-agnostic). Suite 59/59. |
| sprint-v15-16-performance-leak-fix (170 watch.mjs zombies + brain bloat + hook tax) | 7 | 6 | 0 | **CLOSED (86% — Story B deferred to human grant)** — Story A: live-tail PID-tracked cleanup + SessionStart auto-orphan-cleanup (killed 170 zombies pre-sprint, leak source patched). Story C: `aegis-run-rotate.sh` (runs/ 98M → 75M, -24%). Story D: brain-graph debounce 3s → 10s. Story E: `tests/aegis-hook-budget-test.sh` × 3 chains (calibrated baselines: PreBash 350ms, PostBash 700ms, PostEdit 1100ms). Story B (`.*` matcher → `Bash\|Edit\|Write\|MultiEdit\|Task`) DEFERRED — requires human maintainer-grant since guard-write blocks mid-session settings.json edits. Suite 60/60. |
...
| **v15 net deliverable** | **61 selected** | **61 done** | **0** | **CC 2.1.139 + 2.1.141 full alignment + transparency + chain integrity + desktop pings + permission dialog + multi-tenant --cwd + hook friendly-fail + installer robustness + manifest fix + modern-schema PreToolUse + perf/leak fix + diagram-first reflex + skill autodiscover** — v15-02 HYBRID /goal wiring, v15-03 NON-APPLICABLE, v15-04/05 audits NO-OP, v15-06 transparent skill model, v15-07 sprint-plan-chain gate, v15-08 terminalSequence, v15-09 approval-gate schema (initial dual-path), v15-10 multi-tenant cwd, v15-12 hook error classifier, v15-13 install glob fix, v15-14 install manifest (tool packages), v15-15 modern-only PreToolUse, v15-16 perf/leak fix (Story B deferred), v15-17 diagram-first reflex, v15-18A skill autodiscover. |
