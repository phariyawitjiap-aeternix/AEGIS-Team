...
# ASCII shield — monospace-clean (echoes assets/logo/aegis-shield.svg silhouette)
echo -e "${BOLD}${BANNER_COLOR}    .-=~~~~~~~~~~=-.    ${NC}"
echo -e "${BOLD}${BANNER_COLOR}   /  .  .  .  .  .  \\  ${NC}"
echo -e "${BOLD}${BANNER_COLOR}  |      /-\\          | ${NC}"
echo -e "${BOLD}${BANNER_COLOR}  |     / A \\         | ${NC}"
echo -e "${BOLD}${BANNER_COLOR}  |    /-----\\        | ${NC}"
echo -e "${BOLD}${BANNER_COLOR}   \\  .         .   /  ${NC}"
echo -e "${BOLD}${BANNER_COLOR}    \\   .     .    /   ${NC}"
echo -e "${BOLD}${BANNER_COLOR}     \\     .     /     ${NC}"
echo -e "${BOLD}${BANNER_COLOR}      \\         /      ${NC}"
echo -e "${BOLD}${BANNER_COLOR}       \\_______/       ${NC}"
echo ""
echo -e "${BOLD}${BANNER_COLOR}     A E G I S   v${VERSION}   ${BANNER_LABEL}${NC}"
...
if [[ "$UPGRADE" == true ]]; then
    ...
    # v8 -> v9 brain-path migration: if the old _aegis-brain/ tree exists and
    # the new .aegis/brain/ doesn't, copy the contents over before install
    # reshapes the tree. Originals are retained in BACKUP_DIR for rollback.
    if [[ -d "${TARGET_DIR}/_aegis-brain" ]]; then
        info "v8 brain detected at _aegis-brain/ -- migrating to .aegis/brain/"
        cp -r "${TARGET_DIR}/_aegis-brain" "${BACKUP_DIR}/_aegis-brain"
        if [[ ! -d "${TARGET_DIR}/.aegis/brain" ]]; then
            mkdir -p "${TARGET_DIR}/.aegis"
            cp -r "${TARGET_DIR}/_aegis-brain" "${TARGET_DIR}/.aegis/brain"
            success "Brain contents copied to .aegis/brain/ (original preserved in backup)"
        else
            info ".aegis/brain/ already exists -- skipping copy, old _aegis-brain/ retained as-is"
        fi
    fi
    # v8 -> v9 settings-schema backup: explicit side-copy so the user can
    # diff the old bypassPermissions model vs the new scoped-allow/deny.
...
directories=(
    "${TARGET_DIR}/.aegis/brain"
    "${TARGET_DIR}/.aegis/brain/resonance"
    "${TARGET_DIR}/.aegis/brain/learnings"
    "${TARGET_DIR}/.aegis/brain/logs"
    "${TARGET_DIR}/.aegis/brain/retrospectives"
    "${TARGET_DIR}/.aegis/brain/tasks"
    "${TARGET_DIR}/.aegis/brain/skill-cache"
    "${TARGET_DIR}/.aegis/brain/metrics"
    "${TARGET_DIR}/.aegis/brain/learnings/raw"
    ...
    "${TARGET_DIR}/.aegis/brain/sprints"
    "${TARGET_DIR}/.aegis/brain/sprints/current"
    "${TARGET_DIR}/.aegis/brain/handoffs"
    "${TARGET_DIR}/.aegis/brain/backlog"
    "${TARGET_DIR}/skills"
    "${TARGET_DIR}/.claude"
...
cat > "${TARGET_DIR}/.aegis/brain/resonance/project-state.md" <<RESONANCE
...
cat > "${TARGET_DIR}/.aegis/brain/resonance/team-state.md" <<TEAMSTATE
...
runtime_helpers=(
    ...
    "aegis-credential-scan.sh" # v15-28 — credential discovery at intake (ask human once, never mid-work)
)
delivered_tools=0
...
if [[ "$UPGRADE" != true ]]; then
    ...
    echo "[$(date '+%Y-%m-%d %H:%M')] PULSE | status=installed | agents=0 | context=0%" \
      > "${TARGET_DIR}/.aegis/brain/logs/heartbeat.log"

    # Create a welcome sprint (sprint-0) with initial kanban
    mkdir -p "${TARGET_DIR}/.aegis/brain/sprints/sprint-0/daily"

    cat > "${TARGET_DIR}/.aegis/brain/sprints/sprint-0/plan.md" <<PLAN
...
PLAN
    ...
    cat > "${TARGET_DIR}/.aegis/brain/sprints/sprint-0/kanban.md" <<KANBAN
# Sprint 0 — Getting Started
...
KANBAN
    ...
    cat > "${TARGET_DIR}/.aegis/brain/sprints/sprint-0/metrics.json" <<METRICS
{
  "sprint": "sprint-0",
  "started": "$(date +%Y-%m-%d)",
...
METRICS
    ...
    ln -sfn sprint-0 "${TARGET_DIR}/.aegis/brain/sprints/current"

    # Token usage + benchmarks (empty but valid structure)
    cat > "${TARGET_DIR}/.aegis/brain/metrics/token-usage.json" <<'TOKENS'
{
  "sprints": {},
...
TOKENS
    ...
    cat > "${TARGET_DIR}/.aegis/brain/metrics/benchmarks.json" <<'BENCH'
{
  "sprints": {},
  "baseline": null
}
...
# ASCII shield — monospace-clean, green = success
echo -e "${BOLD}${GREEN}    .-=~~~~~~~~~~=-.    ${NC}"
echo -e "${BOLD}${GREEN}   /  .  .  .  .  .  \\  ${NC}"
echo -e "${BOLD}${GREEN}  |      /-\\          | ${NC}"
echo -e "${BOLD}${GREEN}  |     / A \\         | ${NC}"
echo -e "${BOLD}${GREEN}  |    /-----\\        | ${NC}"
echo -e "${BOLD}${GREEN}   \\  .         .   /  ${NC}"
echo -e "${BOLD}${GREEN}    \\   .     .    /   ${NC}"
echo -e "${BOLD}${GREEN}     \\     .     /     ${NC}"
echo -e "${BOLD}${GREEN}      \\         /      ${NC}"
echo -e "${BOLD}${GREEN}       \\_______/       ${NC}"
echo ""
echo -e "${BOLD}${GREEN}     A E G I S   v${VERSION}${NC}"
...
echo ""
echo -e "${BOLD}Directory Structure (v9):${NC}"
echo "  ${TARGET_DIR}/"
echo "  ├── CLAUDE.md              # Hub (load every session)"
echo "  ├── CLAUDE_safety.md       # Safety rules"
echo "  ├── CLAUDE_agents.md       # 11 agents (v9 roster, plus _archived/)"
echo "  ├── CLAUDE_skills.md       # Skill catalog"
echo "  ├── CLAUDE_lessons.md      # Patterns & anti-patterns"
echo "  ├── AEGIS_VERSION          # Installed version pin"
echo "  ├── .aegis/brain/          # v9 single-folder brain"
echo "  │   ├── resonance/         # Identity, ADRs, patterns"
echo "  │   ├── learnings/         # Accumulated knowledge"
echo "  │   ├── retrospectives/    # Session retros"
echo "  │   ├── handoffs/          # Cross-session pickup briefs"
echo "  │   ├── sprints/           # Kanban + plans"
echo "  │   ├── instincts/         # Confidence-scored patterns"
echo "  │   ├── state/             # Transient (gitignored)"
echo "  │   └── logs/              # Agent + hook logs (gitignored)"
echo "  ├── _aegis-output/         # Pipeline outputs (ISO docs, reviews)"
echo "  ├── tools/                 # 60+ helper scripts (aegis-*.sh)"
echo "  ├── skills/                # Skill definitions"
echo "  └── .claude/               # Claude CLI config"
echo "      ├── agents/            # 11 agent prompts + _archived/"
echo "      ├── commands/          # 16 canonical commands (legacy shims removed in v10-05; +aegis-decisions +aegis-goal in v14)"
